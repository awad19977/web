...
export function AddStockForm({
  ...
  const { data: availableUnits = [], isLoading: unitsLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (!response.ok) throw new Error(t('add_stock.failed_fetch_units'));
  ...
  const submitLabel = isEditing
    ...
    : loading
      ? "Adding..."
      : "Add Stock";
  const titleText = isEditing ? t('add_stock.edit_title') : t('add_stock.add_title');
  const submitText = isEditing ? (loading ? t('add_stock.saving') : t('add_stock.save_changes')) : (loading ? t('add_stock.adding') : t('add_stock.add'));
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                >
                  <option value="" disabled={baseUnitOptions.length > 0}>
                    {baseUnitOptions.length ? t('add_stock.select_base_unit') : t('add_stock.no_units_available')}
                  </option>
                ...
                {!unitsLoading && !baseUnitOptions.length ? (
                  <p className="mt-2 text-xs text-amber-600 dark:text-amber-400">
                    {t('add_stock.no_units_found_message')}
                  </p>
                ) : null}
              </div>
              ...
              {canConfigureExtraProduction ? (
                <div className="space-y-3 rounded-lg border border-gray-200 bg-gray-50/60 p-3 dark:border-gray-700 dark:bg-[#262626]">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm font-medium text-gray-700 dark:text-gray-300">
                        {t('add_stock.allow_extra_production')}
                      </p>
                      <p className="text-xs text-gray-500 dark:text-gray-400">
                        {t('add_stock.allow_extra_production_help')}
                      </p>
                    </div>
                  ...
                  <div className="grid gap-2">
                    <label className="text-xs font-medium text-gray-600 dark:text-gray-400">
                      {t('add_stock.max_extra_per_order')}
                    </label>
                    <input
                      type="number"
                      min="0"
                      ...
                      placeholder={t('add_stock.example_placeholder')}
                    />
                  </div>
                </div>
              ) : null}
                ...
                <div className="flex items-center justify-between">
                  ...
                  >
                    {t('add_stock.add_unit')}
                  </button>
                </div>
                <p className="text-xs text-gray-500 dark:text-gray-400">
                  {t('add_stock.define_units', { base: selectedBaseUnit?.name || '?' })}
                </p>

                {conversions.length === 0 ? (
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    {t('add_stock.no_alternate_units')}
                  </p>
                ) : (
                  <div className="space-y-3">
                    {conversions.map((conversion, index) => (
                      ...
                      >
                        <div>
                          ...
                          >
                            <option value="">{t('add_stock.select_unit')}</option>
                            {baseUnitOptions
                        ...
                        <div>
                          <label className="mb-1 block text-xs font-medium text-gray-600 dark:text-gray-400">
                            {t('add_stock.contains_base_units')}
                          </label>
                          <input
                            type="number"
                            min="0"
